...
function parseData() {
    ...
    if (selectedGame === 'yugioh') {
        result = parseYuGiOhData(dataInput);
    } else if (selectedGame === 'onepiece') {
        result = parseOnePieceData(dataInput);
    } else if (selectedGame === 'mtg') {
        result = parseMTGData(dataInput);
    }

    return result;
}

function parseOnePieceData() {
    const dataInput = document.getElementById('dataInput').value.trim();
    if (!dataInput) {
        showError('Input data cannot be empty.');
        return [];
    ...
    for (let i = 0; i < lines.length; i += 4) {
        if (i + 3 >= lines.length) {
            showError('Incomplete data entry found.');
            continue;
        }

        const playerID = lines[i + 1].trim();
        const playerName = lines[i + 2].trim();
        const wins = parseInt(lines[i + 3].split('\t')[0].trim(), 10);

        if (!playerID || !playerName || isNaN(wins)) {
            showError('Invalid data format.');
            continue;
        }

        const checkbox = document.getElementById('specialEventCheckbox'); // Replace 'specialEventCheckbox' with the actual ID of your checkbox
...
function updateTable() {
    ...
    // Determine columns with only zeros
    const columnsWithOnlyZeros = uniqueDates.map(date => {
        return playerData.every(entry => {
    ...
    playerData.forEach(entry => {
        ...
        if (totalPoints > 0) { // Only include players with more than 0 total points
...
function deleteData() {
    const dateToDelete = document.getElementById('dateInput').value;
    if (!dateToDelete) {
        showError('Please select a date to delete.');
        return;
    }

    playerData.forEach(player => {
        player.points.forEach(point => {
            if (point.date === dateToDelete) {
                point.value = 0;
            }
        });
    });
    ...
    updateTable();
    showSuccess(`Data for the selected date (${dateToDelete}) has been deleted.`);
}
